...
mydata_origin <- read.table(file = 'hubs_control.csv',sep = ',',header = T)
...
mydata_origin <- read.table(file = 'hubs_occult.csv',sep = ',',header = T)
mydata <- mydata_origin[,1:4]
mydata$r_order <- factor(x = c(90:1),labels = mydata$node[order(c(1:90),decreasing = T)])
...
grid.arrange(barplot_1,barplot_2,nrow = 1)
